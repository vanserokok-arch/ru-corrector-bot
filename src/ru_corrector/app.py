...
class CorrectionRequest(BaseModel):
    ...
    mode: Literal["min", "biz", "acad"] = Field(
        default="min",
        description="Correction mode: min (minimal), biz (business), acad (academic)"
    )
    ...
    def validate_text_length(cls, v: str) -> str:
        ...
        if len(v) > config.MAX_TEXT_LEN:
            raise ValueError(
                f"Text too long. Maximum length is {config.MAX_TEXT_LEN} characters"
            )
        return v
...
async def correct_endpoint(request: CorrectionRequest):
    ...
    try:
        ...
        changes = sum(1 for a, b in zip(request.text, corrected) if a != b)
        ...
        raise HTTPException(
            status_code=500,
            detail="Error processing text correction"
        )
